...
lemma line_unique' {u v : V} (hu : u ≠ 0) (hv : v ≠ 0) (huv : LinearIndependent K ![u, v])
    (p : Submodule K V) (hp1 : p.finrank = 2)
  ...
  rw [hp1, ← Matrix.range_cons_cons_empty _ _ ![], finrank_span_eq_card huv]
  simp

lemma line_unique {x y : ℙ K V} (hxy : x ≠ y) (p q : Submodule K V) (hp1 : p.finrank = 2)
    (hq1 : q.finrank = 2) (hp2 : x ∈ p.projectivization) (hp3 : y ∈ p.projectivization)
